...
export function EvilEye({
  ...
  useEffect(() => {
    ...
    function updateTargetFromPoint(clientX: number, clientY: number) {
      ...
      mouse.tx = Math.max(-1, Math.min(1, (clientX - centerX) / Math.max(rect.width * 0.5, 1)));
      mouse.ty = Math.max(-1, Math.min(1, -(clientY - centerY) / Math.max(rect.height * 0.5, 1)));
    }
  ...
  return <div className="evil-eye-container" ref={containerRef} />;
}
