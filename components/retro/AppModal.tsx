...
export function AppModal({ open, eyebrow, title, children, footer, closeOnOutside = false, onClose }: AppModalProps) {
  ...
  return (
    ...
    >
      <section className="flex max-h-[min(92dvh,calc(100vh-2rem))] w-full max-w-md flex-col overflow-hidden rounded-[2rem] border border-[#ded8e8]/80 bg-white text-slate-950 shadow-[0_28px_90px_-42px_rgba(49,46,78,0.42)]">
        <div className="flex items-start justify-between gap-4 p-5">
          ...
          <button type="button" onClick={onClose} className="shrink-0 rounded-full bg-slate-100 p-2 text-slate-500 hover:bg-[#f1eef6]" aria-label="Close modal">
            <X className="h-4 w-4" />
          </button>
        </div>
        <div className="min-h-0 overflow-y-auto px-5 pb-5">{children}</div>
        {footer ? <div className="flex shrink-0 flex-wrap gap-2 border-t border-[#ded8e8] bg-[#f7f5f0] p-5">{footer}</div> : null}
      </section>
    </div>
  );
}
